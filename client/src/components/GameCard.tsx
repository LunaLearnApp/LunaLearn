...
const GameCard = ({ game }: GameCardProps) => {
  const gradeBadge = game.minGrade === game.maxGrade 
    ? `Grade ${game.minGrade === 0 ? 'K' : game.minGrade}`
    : `Grades ${game.minGrade === 0 ? 'K' : game.minGrade}-${game.maxGrade}`;
      ...
      <Link href={`/games/${game.id}`}>
        <a className="block">
          <div className="relative">
            ...
            <span className={`absolute top-4 right-4 font-bold px-3 py-1 rounded-full text-sm ${getSubjectColor(game.subject)}`}>
              {game.subject}
            </span>
